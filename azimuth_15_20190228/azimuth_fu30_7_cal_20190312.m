% function [kout_theta]=azimuth_15_cal(fa2_pp,scan_theta)
...
% %   目标出射光路
if FlagDraw==1
    plot([x_temp_min,array_x(1)],[Farest,array_y(1)],'linewidth',2,'color',[.5 .9 .5]);hold on
...
k12=tand(k12_theta);
b12=Y1-k12.*X1;
X2=double( Xr21(b12,k12)); %     可能会修改Xr21，Xr22
Y2=fa(X2);
...
% fb;
% fb2;
scan_thetac=mean(kout_theta-110+scan_theta)
...
% % % % %     %   绘制介质内光路
% % % % %     for n=1:length(X1)
% % % % %         plot([X1(n),X2(n)],[Y1(n),Y2(n)],'r','linewidth',1.2);hold on
% % % % %     end
% % % % %     %   绘制出射光路
% % % % %     for n=1:length(X1)
...
% % % % %     % k_out_theta
% % % % %     hold on
% % % % %     title(['扫描角度是',num2str(-scan_theta+20),'\circ'])
% % % % %     legend off
% % % % %     
% % % % % %     
% % % % % end
